...
public class RequestBook extends JFrame {
    ...
    public RequestBook() {
        ...
        lblNewLabel.setBounds(169, 92, 46, 14);
        ...
        mainPanel.setBounds(63, 95, 290, 240);
        ...
        JLabel durationLabel = new JLabel("How long should the book remain with you?");
        durationLabel.setForeground(new Color(254, 255, 255));
        durationLabel.setFont(new Font("Lucida Grande", Font.PLAIN, 14));
        ...
        borrowButton.addActionListener(clickEvent -> {
            ...
            if(selectedBooks.isEmpty()) {
                JOptionPane.showMessageDialog(panel, "Dear customer, please select a book in order to send a request to the staff");
            } else {
                try {
                    for(String book : selectedBooks) {
                        System.out.println("Trying to borrow -> " + book);
                        addRequestToAdmin(books);
                    }
                    JOptionPane.showMessageDialog(panel, "Request has been sent!");
                } catch (Exception error) {
                    JOptionPane.showMessageDialog(panel, "An error has occured while sending the request!");
...
//	        jFrame.add(scrollPane_1);
    ...
    public void addRequestToAdmin(BooksDB book) {
